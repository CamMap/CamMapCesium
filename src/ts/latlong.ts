...
export function calcLatLong([lat, long]: [number, number], distance: number, bearing: number): [number, number] {
    ...
    const long1 = Cesium.Math.toRadians(long);

    const lat2: number = Math.asin(Math.sin(lat1) * Math.cos(distance / RADIUS_EARTH) + Math.cos(lat1) * Math.sin(distance / RADIUS_EARTH) * Math.cos(bearing));

    const long2 = long1 + Math.atan2(Math.sin(bearing) * Math.sin(distance / RADIUS_EARTH) * Math.cos(lat1), Math.cos(distance / RADIUS_EARTH) - Math.sin(lat1) * Math.sin(lat2));
